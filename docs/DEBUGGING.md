...
- `staff/content-screen.js`: content/ID Create, Read, Update, and Delete from one searchable screen. The Staff manager reads only `directory_entries.category = Barangay Staff` and assigns that category automatically on create/save. The Functionaries manager reads the same table while excluding Contact and Barangay Staff, and accepts the exact local functionary group heading plus an optional photo/icon upload. Read uses a details dialog; writes remain allowlisted by `data/contracts.js`; deletion uses an in-page confirmation before the existing service/RLS call.
