...
- `design/public-renderer.js` also groups the full Officials route by the existing `position`: Punong Barangay; Barangay Kagawad; IPMR/Secretary/Treasurer; then SK Chairperson, SK Kagawad, and SK Secretary/Treasurer. Unknown positions remain visible under the appropriate Other tier. The homepage keeps a compact preview, and every layout remains responsive without duplicating Officials records or CRUD.
